...
const IntervencionFamiliarDetail = () => {
  const { id } = useParams();
  ...
  const loadIntervencion = async () => {
    setLoading(true);
    try {
      const data = await IntervencionFamiliarService.getIntervencionFamiliarById(id);
      if (!data) {
        message.error('No se encontró la intervención especificada');
        navigate('/intervenciones/familiares');
        return;
      }
  ...
  const handleBack = () => {
    if (intervencion?.familiaId) {
      navigate(`/familias/${intervencion.familiaId}/intervenciones`);
    } else {
      navigate('/intervenciones/familiares');
    }
  };
  ...
  const handlePrint = () => {
    ...
    // Crear lista de miembros presentes
    let miembrosHtml = '<p>No hay miembros registrados como presentes en esta intervención</p>';
    if (intervencion.miembrosPresentes && intervencion.miembrosPresentes.length > 0) {
      miembrosHtml = '<ul>';
      intervencion.miembrosPresentes.forEach(miembro => {
        miembrosHtml += `<li><strong>${miembro.nombreCompleto || miembro.documentoEstudiante}</strong> - ${miembro.relacion || 'No especificada'}</li>`;
      });
      miembrosHtml += '</ul>';
    }
    ...
    const printableContent = `
      <html>
        <head>
          <title>Intervención Familiar - ${intervencion.nombreFamilia || intervencion.familiaId}</title>
          ${printStyles}
        </head>
        <body>
          <h1>Detalle de Intervención Familiar</h1>
          ...
          <div class="header">
            ...
            <div>
              <strong>Familia:</strong> ${intervencion.nombreFamilia || intervencion.familiaId}
            </div>
          </div>
          ...
          <div class="section">
            <div class="section-title">Miembros presentes</div>
            ${miembrosHtml}
          </div>
  ...
  const handleDelete = () => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await IntervencionFamiliarService.deleteIntervencionFamiliar(id);
  ...
  const handleEditModalSubmit = async (values) => {
    try {
      await IntervencionFamiliarService.updateIntervencionFamiliar(id, values);
  ...
  return (
    <Card>
      <Space direction="vertical" style={{ width: '100%' }}>
        <Space>
          ...
          <Title level={3} style={{ margin: 0 }}>
            Detalles de Intervención Familiar
          </Title>
        </Space>
        ...
        >
          <Descriptions.Item label="Familia">
            {intervencion.nombreFamilia || intervencion.familiaId}
          </Descriptions.Item>
          ...
          <Descriptions.Item label="Tipo">
            <Tag color={
              ...
              intervencion.tipo === 'seguimiento' ? 'purple' :
              'default'
            }>
        ...
        <Divider orientation="left">Miembros presentes</Divider>
        {intervencion.miembrosPresentes && intervencion.miembrosPresentes.length > 0 ? (
          <List
            itemLayout="horizontal"
            dataSource={intervencion.miembrosPresentes}
            renderItem={(miembro, index) => (
              <List.Item key={generateUniqueKeyForItem(miembro, index, 'miembro-presente')}>
                <List.Item.Meta
                  avatar={<Avatar icon={<UserOutlined />} />}
                  title={miembro.nombreCompleto || miembro.documentoEstudiante}
                  description={miembro.relacion || 'No especificada'}
                />
              </List.Item>
            )}
          />
        ) : (
          <Paragraph>No hay miembros registrados como presentes en esta intervención</Paragraph>
        )}
      ...
      <Modal
        title="Editar Intervención Familiar"
      ...
      >
        <IntervencionFamiliarForm
          ...
          familiaId={intervencion?.familiaId}
          useUniqueKeys={true}
        />
      </Modal>
    </Card>
...
export default IntervencionFamiliarDetail;
